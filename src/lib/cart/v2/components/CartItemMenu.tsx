...
import { CartItem, CartItemVariation } from '@/lib/cart/v2/cart-types';
...
export const CartItemMenu: React.FC<CartItemMenuProps> = ({ item }) => {
  ...
  return (
    <div className="py-10 px-4 items-container border-b border-b-[#C1C1C1]">
      <div className="text-[14px] grid grid-cols-7 gap-5 ">
        ...
        <div className="leading-4 col-span-5 flex flex-col gap-1">
          ...
          <div className="text-[#666666] flex space-x-[2px]">
            <span>
              {formatCurrency(item?.prices?.price) + ' x ' + item?.quantity}
            </span>
          </div>

          <span className="flex text-[#111111]">
            Código: {item?.sku?.toString()}
          </span>
          ...
          {item?.variation &&
            item?.variation?.map((item: CartItemVariation, key: number) => {
              return (
