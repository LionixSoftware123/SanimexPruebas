...
import { addCartAction } from '@/lib/cart/v2/cart-actions';
import {
  locationStore,
  setCity,
...
const AddCartButton: React.FC<AddCartButtonProps> = ({
  ...
  const { updateCart, refetchCart } = useCartHook();
  const { addToast } = useToasts();
  const router = useRouter();
  const isLimited = !!LimitedProductsGeo['products'].includes(
    router?.query?.product as string,
  );

  const [callAddCart, loading] = useCallAction(addCartAction, {
    onCompleted: (data) => {
  ...
  const { postalCode, city } = useStore(locationStore);
  ...
  const onAddToCart = () => {
    if (!city && isLimited) {
      ...
      return addToast('Debe tener la geolocalización activa.', {
        appearance: 'error',
      });
    }

    if (isLimited && postalCode) {
      return addToast('El producto no está disponible en tu región.', {
        appearance: 'error',
      });
    }
    ...
    callAddCart({
      id: variationProduct ? variationProduct?.databaseId : product?.databaseId,
      quantity,
    });
