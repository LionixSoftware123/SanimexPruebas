...
import {
  cleanProductAttributeBoxOptionUpsale,
  getProductAttributeBoxUpsale,
} from '../../cart-actions';

type ProductBoxCalculatorUpsaleProps = {
  product?: UpsaleItem;
...
const ProductBoxCalculatorUpsale: React.FC<ProductBoxCalculatorUpsaleProps> = ({
  product,
  quantity = 0,
  onQuantity = () => {},
  active = true,
  ...
  const calculateData = () => {
    const stockQuantity = product?.quantity_limits?.maximum;

    const productAttributeBox = getProductAttributeBoxUpsale(
    ...
    const quantityBox =
      mtr / cleanProductAttributeBoxOptionUpsale(productAttributeBox);
...
export default ProductBoxCalculatorUpsale;
