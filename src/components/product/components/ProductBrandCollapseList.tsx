...
import { useFetchAllMarcasQuery } from '@/utils/types/generated';

type ProductBrandCollapseListProps = {
  onFilter?: (item: string[]) => void;
};

const ProductBrandCollapseList: React.FC<ProductBrandCollapseListProps> = ({
  onFilter,
}) => {
  const [open, setOpen] = useState<boolean>(false);
  const [selected, setSelected] = useState<string[]>([]);
  const { data, loading } = useFetchAllMarcasQuery({
    variables: {
      first: 100,
    },
  });
  ...
  return (
    <div className="w-full rounded-[5px] border-b-[#0071CE] border-b-[3px]">
      <div className=" font-Century-Gothic">
        ...
        >
          <div className="font-Century-Gothic text-[#555555]">Marcas</div>
        ...
        {open && (
          <div className="p-[20px] max-h-[350px] overflow-auto">
            {loading ? (
              ...
              <>
                {data?.allPaMarcas?.edges?.map((listItem, i) => (
                  <div
                    ...
                    dangerouslySetInnerHTML={{
                      __html: listItem.node.name as string,
                    }}
                  ></div>
                ))}
              </>
...
export default ProductBrandCollapseList;
