...
const ProductBranchFilter: React.FC<ProductBranchFilterProps> = ({
  onFilter,
  taxonomyField = '',
  ...
  useEffect(() => {
    ...
    if (existsInTaxonomy && !hasRun.current) {
      if (onFilter && queryArray) {
        onFilter(queryArray);
      }
      setSelected(queryArray);
      hasRun.current = true;
    }
  }, [taxonomy, router.query.filter]);

  const handleSelection = (slug: string) => {
    let updatedSelected: string[];
    if (selected.includes(slug)) {
      updatedSelected = selected.filter((item) => item !== slug);
    } else {
      updatedSelected = [...selected, slug];
    }
    ...
    router.push(
      {
        ...
        query: { ...router.query, filter: updatedSelected },
      },
      undefined,
      { shallow: true },
    );
                ...
                {taxonomy?.map((listItem, i) => (
                  <div
                    className={`cursor-pointer ${
                      selected[selected.length - 1] === listItem?.slug &&
...
export default ProductBranchFilter;
