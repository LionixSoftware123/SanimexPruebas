...
import { useFetchAllMaterialQuery } from '@/utils/types/generated';

type ProductMaterialCollapseListProps = {
  onFilter?: (item: string[]) => void;
};

const ProductMaterialCollapseList: React.FC<
  ProductMaterialCollapseListProps
> = ({ onFilter }) => {
  const [open, setOpen] = useState<boolean>(false);
  const [selected, setSelected] = useState<string[]>([]);

  const { data, loading } = useFetchAllMaterialQuery({
    variables: {
      first: 100,
    },
  });
    ...
    <div className="w-full rounded-[5px] border-b-[#0071CE] border-b-[3px]">
      <div className=" font-Century-Gothic">
        <div className="bg-[#F5F5F5] w-full flex h-[40px] items-center p-[20px] justify-between">
          <div className="font-Century-Gothic text-[#555555]">Materiales</div>
        ...
        {open && (
          <div className="p-[20px] max-h-[350px] overflow-auto">
            {loading ? (
              ...
              <>
                {data?.allPaMaterial?.edges?.map((listItem, i) => (
                  <div
...
export default ProductMaterialCollapseList;
