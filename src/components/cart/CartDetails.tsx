...
import { Cart as CartType, ProductCategory } from '@/utils/types/generated';
...
const CartDetails: React.FC<CartDetailsProps> = ({ cart }) => {
  ...
  const isFindTina = cart?.contents?.nodes?.find((node) => {
    return node.product?.node?.productCategories?.nodes.find(
  ...
  const renderDeliveryInfo = () => {
    if (isFindTina) {
      return (
        <div>
          * Producto exclusivo bajo pedido. Tiempo de entrega (25 a 30 días
          naturales)
        </div>
      );
    }

    return <div className="mb-2">* Envío a domicilio de 5 a 7 días</div>;
  };
    ...
    <div className="border border-[#707070] mb-6">
      ...
      <div className="p-4 text-[14px] ">
        <div className=" text-[#666666] flex justify-between items-center font-Century-Gothic text-[14px]">
          <div>Subtotal</div>
          <div>{cart?.subtotal} MXN</div>
        </div>
        <div className=" text-[#666666] flex justify-between items-center font-Century-Gothic text-[14px]">
          <div>IVA</div>
          <div>{cart?.totalTax} MXN</div>
        </div>
