...
type ProductFavoriteCustomProps = {
  product?: ExtendedProductCustom;
  onSuccess?: (favorites: number[]) => void;
};

const ProductFavoriteCustom: React.FC<ProductFavoriteCustomProps> = ({
  product,
  onSuccess,
}) => {
  ...
  const [savePosts] = useSaveFavoriteProductMutation({
    onCompleted: (data) => {
      ...
      if (
        ...
        newFavoriteProducts = [
          ...favoriteProducts,
          (product as any)?.external_id || product?.databaseId || product?.id,
        ];
        setIsFavorite(true);
      } else {
        let productId: any;
        ...
        if (product?.id && !isNaN(Number(product?.id))) {
          productId =
            Number((product as any).external_id) ||
            product?.databaseId ||
            product?.id;
        } else {
          productId =
  ...
  useEffect(() => {
    if (favoriteProducts) {
      const findProduct = favoriteProducts?.find(
        (favoriteProductId) =>
          favoriteProductId === (product?.databaseId || product?.id),
      );
            ...
            : savePosts({
                variables: {
                  input: {
                    ...
                    databaseProductId: product?.id || product?.databaseId,
                  },
                },
              })
        }
        className="w-[33px] h-[33px] place-items-center flex border border-[#F1F1F1] rounded-full bg-white cursor-pointer "
      >
        <div className="flex justify-center mx-auto pt-[2px]">
          {isFavorite ? (
            <>
              <IconFavoriteRed
                style={{
...
export default ProductFavoriteCustom;
