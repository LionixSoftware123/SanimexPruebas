...
const FavoriteComponent: React.FC<FavoriteComponentProps> = ({
  ...
  const [savePosts] = useSaveFavoriteProductMutation({
    onCompleted: (data) => {
      ...
      if (
        ...
        newFavoriteProducts = [
          ...favoriteProducts,
          product?.databaseId || product?.external_id,
        ];
        setIsFavorite(true);
      } else {
  ...
  const ProductId = product?.databaseId || product?.external_id;

  useEffect(() => {
    if (favoriteProducts) {
      const findProduct = favoriteProducts?.find((favoriteProductId) => {
        console.log(
          'favoriteProductId',
          favoriteProductId,
          'ProductId',
  ...
  }, [product, favoriteProducts, ProductId]);
